...
func (r *saleProductRepo) GetList(ctx context.Context, req *models.GetListSaleProductRequest) (*models.GetListSaleProductResponse, error) {
	...
	if len(req.Search) > 0 {
		where += " AND product_name ILIKE" + " '%" + req.Search + "%'"
	}
	...
	for rows.Next() {
		...
		err = rows.Scan(
			...
			&saleProduct.Barcode,
			&saleProduct.RemainingQuantity,
			&saleProduct.Quantity,
			&saleProduct.AllowDiscount,
			&saleProduct.DiscountType,
			&saleProduct.Discount,
			&saleProduct.Price,
			&saleProduct.TotalAmount,
		...
		saleProduct.UpdatedAt = updatedAt.String
