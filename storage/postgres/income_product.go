...
func (r *incomeProductRepo) GetList(ctx context.Context, req *models.GetListIncomeProductRequest) (*models.GetListIncomeProductResponse, error) {
	...
	if len(req.Search) > 0 {
		where += " AND (product_name ILIKE" + " '%" + req.Search + "%' OR barcode ILIKE '%" + req.Search + "%')"
	}
	...
	for rows.Next() {
		...
		incomeProduct.IncomeID = IncomeID.String
		incomeProduct.CategoryID = CategoryID.String
		incomeProduct.Quantity = Quantity.Int64
		incomeProduct.IncomePrice = IncomePrice.Float64
