...
func (r *remainderRepo) GetList(ctx context.Context, req *models.GetListRemainderRequest) (*models.GetListRemainderResponse, error) {
	...
	if len(req.Search) > 0 {
		where += fmt.Sprintf(" AND (product_name ILIKE '%%%s%%' OR barcode ILIKE '%%%s%%')", req.Search, req.Search)
	}
	...
	for rows.Next() {
		...
		remainder.BranchID = branchID.String
		remainder.CategoryID = categoryID.String
		resp.Remainder = append(resp.Remainder, &remainder)
	}
